...
- (NSString*)replaceColonsInString:(NSString*)timeFormat withTimeSeparator:(unichar)timeSep {
    if (timeSep != ':') {
        ...
        [timeFormatMutable replaceOccurrencesOfString:@":" withString:[NSString stringWithCharacters:&timeSep length:1U] options:NSBackwardsSearch | NSLiteralSearch range:(NSRange){0UL, [timeFormat length]}];
    ...
    switch (self.format) {
        ...
        default:
            [NSException raise:NSInternalInconsistencyException
                        format:@"self.format was %lu, not calendar (%d), week (%d), or ordinal (%d)", (unsigned long)self.format, ISO8601DateFormatCalendar, ISO8601DateFormatWeek, ISO8601DateFormatOrdinal];
            return nil;
    }
}
...
- (NSString*)stringFromDate:(NSDate*)date formatString:(NSString*)dateFormat timeZone:(NSTimeZone*)timeZone {
    ...
    if (includeTime) {
        long offset = [timeZone secondsFromGMT];
